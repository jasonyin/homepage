...
import {JYFoundation} from '../../base';
import {cssClasses, strings, numbers} from './constants';
import {clamp, bezierProgress} from '../util';

export default class JYSimpleMenuFoundation extends JYFoundation {
  ...
  constructor(adapter) {
    super(Object.assign(JYSimpleMenuFoundation.defaultAdapter, adapter));
  ...
  init() {
    const {ROOT, OPEN} = JYSimpleMenuFoundation.cssClasses;
  ...
  applyTransitionDelays_() {
    const {BOTTOM_LEFT, BOTTOM_RIGHT} = JYSimpleMenuFoundation.cssClasses;
    const numItems = this.adapter_.getNumberOfItems();
    const {height} = this.dimensions_;
    const transitionDuration = JYSimpleMenuFoundation.numbers.TRANSITION_DURATION_MS / 1000;
    const start = JYSimpleMenuFoundation.numbers.TRANSITION_SCALE_ADJUSTMENT_Y;
  ...
  animationLoop_() {
    ...
    const {TRANSITION_DURATION_MS, TRANSITION_X1, TRANSITION_Y1, TRANSITION_X2, TRANSITION_Y2,
        TRANSITION_SCALE_ADJUSTMENT_X, TRANSITION_SCALE_ADJUSTMENT_Y} = JYSimpleMenuFoundation.numbers;
    ...
    if (currentTime < 1) {
      ...
      this.animationRequestId_ = 0;
      this.running_ = false;
      this.adapter_.removeClass(JYSimpleMenuFoundation.cssClasses.ANIMATING);
    }
  }
  ...
  open({focusIndex = null} = {}) {
    ...
    this.adapter_.addClass(JYSimpleMenuFoundation.cssClasses.ANIMATING);
    this.animationRequestId_ = requestAnimationFrame(() => {
      this.dimensions_ = this.adapter_.getInnerDimensions();
      this.applyTransitionDelays_();
      this.autoPosition_();
      this.animateMenu_();
      this.adapter_.addClass(JYSimpleMenuFoundation.cssClasses.OPEN);
  ...
  close() {
    ...
    requestAnimationFrame(() => {
      ...
      this.animateMenu_();
      this.adapter_.removeClass(JYSimpleMenuFoundation.cssClasses.OPEN);
    });
    this.isOpen_ = false;
