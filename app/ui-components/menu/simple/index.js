...
import {JYComponent} from '../../base';
import JYSimpleMenuFoundation from './foundation';
import {getTransformPropertyName} from '../util';

export {JYSimpleMenuFoundation};

export class JYSimpleMenu extends JYComponent {
  static attachTo(root) {
    return new JYSimpleMenu(root);
  }
  ...
  get itemsContainer_() {
    return this.root_.querySelector(JYSimpleMenuFoundation.strings.ITEMS_SELECTOR);
  }
  ...
  get items() {
    ...
    return [].slice.call(itemsContainer.querySelectorAll('.JY-list-item[role]'));
  }

  getDefaultFoundation() {
    return new JYSimpleMenuFoundation({
      ...
      hasAnchor: () => this.root_.parentElement && this.root_.parentElement.classList.contains('JY-menu-anchor'),
      ...
      notifySelected: (evtData) => this.emit('JYSimpleMenu:selected', {
        index: evtData.index,
        item: this.items[evtData.index],
      }),
      notifyCancel: () => this.emit('JYSimpleMenu:cancel'),
      saveFocus: () => {
